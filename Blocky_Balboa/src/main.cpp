...
int main()
{
	cece::show_console_window(false);
	...
	Mesh mesh("heat_icososphere", texture_source);
	...
	while (window.isOpen())
	{
		...
		angle += g_delta_time * 0.8f;
		shader_program.update_uniform(UniformType::MAT4, "model_matrix", cece::createYRotationMatrix(angle).c_arr());
